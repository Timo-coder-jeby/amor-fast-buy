...
import { HolidayGiftBoxesProps } from "@types";
...
const HolidayGiftBoxes = ({ giftBoxes }: HolidayGiftBoxesProps) => {
  ...
  // 获取 badge 颜色（根据折扣百分比）
  const getBadgeColor = (discountPercentage: number) => {
    if (discountPercentage >= 15) return "bg-red-500";
    if (discountPercentage >= 10) return "bg-orange-500";
    return "bg-green-500";
  };

  // 获取盲盒图标样式（根据索引）
  const getBoxIconId = (index: number) => (index % 3) + 1;
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {
            giftBoxes.map((box, index) => (
              <div key={box.ruleId} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 group border border-transparent hover:border-pink-500">
                {/* Image Section */}
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 right-4">
                    <div className={`${getBadgeColor(box.discountPercentage)} text-white px-4 py-2 rounded-lg text-center text-sm font-medium mb-4`}>
                      {box.theme}
                    </div>
                  </div>
                </div>
                ...
                {/* Content Section */}
                <div className="p-6">
                  <h3 className="text-xl font-bold text-gray-900 group-hover:text-pink-500 mb-3 transition-colors duration-300">{box.name}</h3>
                  <p className="text-sm text-gray-600 mb-4 leading-relaxed">{box.description}</p>

                  {/* Products Preview */}
                  <div className="mb-6">
                    <div className="space-y-2">
                      {box.products.slice(0, 2).map((product) => (
                        <div key={product.productId} className="flex items-center text-sm text-gray-600">
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="text-3xl font-bold text-pink-600">
                        {formatPrice(box.totalCurrentPrice, box.currency)}
                      </span>
                      <span className="text-xl text-gray-400 line-through">
                        {formatPrice(box.totalOriginalPrice, box.currency)}
                      </span>
                    </div>
                    <div className="text-sm text-green-600 font-medium">
                      Save {formatPrice(box.savedAmount, box.currency)}
                    </div>
                  </div>
